		...
		<div id="contents">
			<form action="${actionUrl}" method="post" id="frm" name="frm" onsubmit="return regist()">
				...
				<table class="chart2">
					...
					<tbody>
						...
						<tr>
							...
							<td>
								<input type="text" id="useBeginTime" class="timepicker" name="useBeginTime" title="운영시작일 "  value="<c:out value="${result.useBeginTime}"/>" readonly="readonly"/>
								~ <input type="text" id="useEndTime" class="timepicker" name="useEndTime" title="운영종료일"  value="<c:out value="${result.useEndTime}"/>" readonly="readonly"/>
							</td>
						</tr>
				...
				<div class="btn-cont ar">
					<c:choose>
						<c:when test="${not empty searchVO.resveId}">
							<c:url var="uptUrl" value="/admin/rsv/rsvUpdate.do${_BASE_PARAM}">
							...
							<a href="${delUrl}" id="btn-del" class="btn spot">삭제</a>
						</c:when>
